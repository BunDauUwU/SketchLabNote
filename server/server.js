  ...
  tryCreateMatch(mode) {
    ...
    while (queue.length >= 2) {
      ...
      const match = {
        ...
        roundStarterIndex: 0, ended: new Set(), weather: createWeatherPlan(),
        started: false, timer: null, actionDeadlineEpochMs: 0,
      };
  ...
  gameCommand(socket, payload) {
    ...
    if (commandType === "EndRound") {
      ...
      if (match.ended.size === 2) {
        this.broadcast(match, "GameEvent", { matchId: match.id, events: [{ eventType: "RoundEnded", round: match.round }] });
        match.round += 1;
        match.ended.clear();
        ...
        this.broadcast(match, "GameEvent", { matchId: match.id, events: [{ eventType: "RoundStarted", round: match.round }, ...weatherEvents] });
        ...
        this.broadcast(match, "GameEvent", { matchId: match.id, events: [{ eventType: "RoundEnded", playerIndex }] });
      }
